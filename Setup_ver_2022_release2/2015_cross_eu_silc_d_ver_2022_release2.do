* 2015_cross_eu_silc_d_ver_2022_release2.do 
*
...
* Country files are delivered in the format UDB_l*country_stub*15D.csv
* 
* (c) GESIS 2023-02-01
* 
* PLEASE NOTE
...
*
* Pforr, Klaus and Johanna Jung (2023): 2015_cross_eu_silc_d_ver_2022_release2.do.
...
local log_file "$log/eusilc_2015_d" ;
...
local stata_file "$log/eusilc_2015_d_cs" ;
...
foreach CC in AT BE BG CH CY CZ DE DK EE EL ES FI FR HR HU IE IS IT LT LU LV MT NL NO PL PT RO RS SE SI SK UK { ;
      cd "$csv_path/`CC'/2015" ;		
	  import delimited using "UDB_c`CC'15D.csv", case(upper) asdouble clear ;
	  * In DE, NL, PT and RS DB040 is missing and read as numeric.
...
* No info on region is available for DE, NL, PT ;
...
12 "order on sampl.frame is fixed for all EU-SILC survey years; PSUs have an unequal probability of selection (within explicit strata)"
21 "order on sampling frame may change over time; PSUs have an equal probability of selection (within explicit strata)"
22 "order on sampling frame may change over time; PSUs have an unequal probability of selection (within explicit strata)"
;
...
1 "filled"
-7 "Not applicable: DB010 ≠ last year"
; 
...
label data "Household register file 2015" ;
